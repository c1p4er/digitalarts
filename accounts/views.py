...
from .forms import RegistrationForm, UserForm, UserProfileForm, SupplierRegistrationForm, SupplierProfile
from .models import Account, UserProfile, Supplier
from carts.models import Cart, CartItem
from carts.views import _cart_id
from orders.models import Order, OrderProduct
from appointment.models import Service, Appointment, Payment
from appointment.forms import AppointmentForm, PaymentForm
...
def registerSupplier(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            user = Supplier.objects.create_user(first_name=first_name, last_name=last_name, email=email, username=username,
                                               address=address, city=city, county=county, password=password, status='pending')
            user.phone_number = phone_number
    ...
    return render(request, 'accounts/supplier/register.html', context)
...
def loginsupplier(request):
    if request.method == 'POST':
        ...
        if user is not None:
            if user.status == 'accepted' and user.is_vendor == True:
                login(request, user)
    ...
    return render(request, 'accounts/supplier/login.html')
...
def dashboard(request):
    ...
    try:
        ...
        userprofile.save()
    pending_appointments = Appointment.objects.filter(user=request.user, status='pending').count()
